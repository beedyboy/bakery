...
function VerifyVariableSchedule_Update($columns)
{
    ...
    $sql_same_period="SELECT cp.COURSE_PERIOD_ID FROM course_periods cp,course_period_var cpv  WHERE  days = '".$columns[DAYS]."' AND period_id='".$per_id."' AND ROOM_ID=$columns[ROOM_ID] AND TEACHER_ID=$teacher";
...
function VerifyStudentSchedule($course_RET,$student_id='')
{
    ...
    {
        $schedule_exist = DBGet(DBQuery("SELECT *  FROM `schedule` WHERE `syear` =".$course_RET[1]['SYEAR']." AND `school_id` =".$course_RET[1]['SCHOOL_ID']." AND `student_id` =".$student_id." AND `course_id` =".$course_RET[1]['COURSE_ID']." AND `course_period_id` = ".$course_RET[1]['COURSE_PERIOD_ID']));
